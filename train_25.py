  ...
  with tf.Graph().as_default():
        ...
        with tf.device('/gpu:%d' % gpu_index):
          with tf.name_scope('%s_%d' % ('dextro-research', gpu_index)) as scope:
            ...
            logit = c3d_model.inference_c3d(
                            images_placeholder[i_run * FLAGS.batch_size * gpu_num + gpu_index * FLAGS.batch_size: i_run * FLAGS.batch_size * gpu_num + (gpu_index + 1) * FLAGS.batch_size,:,:,:,:],
                            0.5,
                            FLAGS.batch_size,
                            weights,
                            biases
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=False):
      ...
      for step in xrange(FLAGS.max_steps):
        ...
        train_images, train_labels = train_sample.batch(size = FLAGS.batch_size * gpu_num * RUN)
        ...
        if (step) % 10 == 0 or (step + 1) == FLAGS.max_steps:
          ...
          val_images, val_labels = test_sample.batch(size = FLAGS.batch_size * gpu_num * RUN)
          summary, acc = sess.run(
                          [merged, accuracy],
                          feed_dict={
